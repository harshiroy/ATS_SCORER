...
- For duration_months,calculate the number of months between start_date and end_date. If end_date is "Present" or "Current",calculate from statr_date to now.
...
Important instructions:
-required_skills:skills explicitly stated as rrequired or must-have.
-preferred_skills:skills stated as preferred,nice-to-have, or bonus.
-keywords:extract ALL important terms an ATS system would match against,icluding skills,technologies,certifications,and domain terms.
- Return ONLY valid JSON. No markdown code fences,no explanation.
...
def parse_job_description(raw_text:str)->Dict:
    client=_get_client()
    prompt=JD_SYSTEM_PROMPT.format(raw_text=raw_text)
...
def _validate_jd_result(result:dict)->dict:
    ...
    for exp in result.get("expierence",[]):
        if not isinstance(exp,dict):
            continue
        exp.setdefault("job_title","")
        exp.setdefault("company","")
