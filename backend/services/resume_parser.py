...
def _exttract_pdf_hyperlinks(file_data:bytes)->str:
    urls=[]
    try:
...
def _extract_pdf_with_pdfplumber(file_data:bytes)->str:
    ...
    hyperlinks=_exttract_pdf_hyperlinks(file_data)
    if hyperlinks:
        text=text.strip()+'\n'+hyperlinks
...
def extract_text_from_doc(file_path:bytes)->str:
    raise FileParsingError(
...
def parse_resume_file(file_data:bytes,filename:str)->Tuple[str,dict]:
    ...
    try:
        ...
        raise FileValidationError(
            'Could not validate the upload file. PLease ensure it is a valid PDF or DOCX.'
        )from e
